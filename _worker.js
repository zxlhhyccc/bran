						...
						if (订阅类型 === 'mixed') {
							...
							if (!url.searchParams.has('sub') && config_JSON.优选订阅生成.local) { // 本地生成订阅
								const 完整优选列表 = config_JSON.优选订阅生成.本地IP库.随机IP ? (
									await 生成随机IP(request, config_JSON.优选订阅生成.本地IP库.随机数量, config_JSON.优选订阅生成.本地IP库.指定端口, (config_JSON.协议类型 === 'ss' ? config_JSON.SS.TLS : true))
							...
							订阅内容 = 其他节点LINK + 完整优选IP.map(原始地址 => {
								...
								if (协议类型 === 'ss') {
									...
									return `${协议类型}://${btoa(config_JSON.SS.加密方式 + ':00000000-0000-4000-8000-000000000000')}@${节点地址}:${节点端口}?plugin=v2${encodeURIComponent('ray-plugin;mode=websocket;host=example.com;path=' + 完整节点路径 + (config_JSON.SS.TLS ? ';tls' : ''))}#${encodeURIComponent(节点备注)}`;
...
async function 处理WS请求(request, yourUUID) {
	const 请求URL = new URL(request.url), WS套接字对 = new WebSocketPair();
